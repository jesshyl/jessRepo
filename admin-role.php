...
@$lim = sprintf(" order by id desc LIMIT %d, %d", 
					@$startRow, @$maxRows);                    //输出maxRow行
@$query_Recordset1 = @$all.@$where;
if(isset($_GET['recordset1'])){
	@$query_Recordset1 = urldecode($_GET['recordset1']);
}
@$query_limit_Recordset1=@$query_Recordset1.@$lim;
@$Recordset1 = $mysql->query(@$query_limit_Recordset1) or die(mysql_error());
@$totalRows = $mysqli->num_rows();
...
<div class="page-container">
	<div class="cl pd-5 bg-1 bk-gray"> <span class="l"><a class="btn btn-primary radius" href="javascript:;" onclick="admin_role_add('添加管理员','admin-role-add.php','800')"><i class="Hui-iconfont">&#xe600;</i> 添加管理员</a> </span> <span class="r">共有数据：<strong><?php ?></strong> 条</span> </div>
