...
@$where="";$case="";@$datemin="";@$datemax="";
if(isset($_POST['datemin'])){
	$status = trim($_POST['status_select']);
	$datemin = trim($_POST['datemin']);
	$datemax = trim($_POST['datemax']);
	if($status!=9||$datemin!=null||$datemax!=null){
		$where = " where ";
	}
	if($datemin!=""){
		if($datemax!=""){
			if($status!=9){//都不为空
				$case = "rec_time >= '".$datemin."' and rec_time <= '".date('Y-m-d',strtotime($datemax." + 1 day"))."' and status = ".$status;
			}else{//status空
				$case = "rec_time >= '".$datemin."' and rec_time <= '".date('Y-m-d',strtotime($datemax." + 1 day"))."'";
			}
		}else{
			if($status!=9){//datemax空
				$case = "rec_time >= '".$datemin."' and status = ".$status;
			}else{//datemax和status空
				$case = "rec_time >= '".$datemin."'";
			}
		}
	}else{
		if($datemax!=""){
			if($status!=9){//datemin空
				$case = "rec_time <= '".date('Y-m-d',strtotime($datemax." + 1 day"))."' and status = ".$status;
			}else{//datemin和status空
				$case = "rec_time <= '".date('Y-m-d',strtotime($datemax." + 1 day"))."'";
			}
		}else{
...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 历史数据管理 <span class="c-gray en">&gt;</span> 历史列表 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<form method="post" name="form1" id="form1" action="event-list.php">
<div class="page-container">
	...
	<div class="cl pd-5 bg-1 bk-gray mt-20" style="margin-bottom: 10px"> <span class="l">共有&nbsp;<font style="font-weight: bold"><?php echo @$totalRows?></font>&nbsp;条数据，&nbsp;当前为第&nbsp;<font style="font-weight: bold;color:blue;"><?php echo @$pageNum+1;?></font>&nbsp;页&nbsp;&nbsp; </span>
...
$(function(){
	...
	<?php
		...
		while($event = @$Recordset1->fetch_assoc()){?>
			$("#table1 tbody").append("<tr class = 'text-c'><td><?php echo $st?></td><td><a title='编辑' href='javascript:;' onclick='event_edit(\"编辑\",\"event-edit.php?eid=<?php echo $event['id']?>\",\"4\",\"\",\"510\")' class='ml-5' style='text-decoration:none'><i class='Hui-iconfont'>&#xe6df;</i></a><a title='删除' href='javascript:;' onclick='event_del(\"<?php echo $event['id'];?>\")' class='ml-5' style='text-decoration:none'><i class='Hui-iconfont'>&#xe6e2;</i></a></td><td><?php echo $event['segment_num']?></td><td><a title='传感器' href='javascript:;' onclick='sensor_show(\"查看传感器\",\"sensor-show.php?snum=<?php echo $event['sensor_num']?>\",\"4\",\"\",\"510\")' ><?php echo $event['sensor_num']?></td></a><td><?php echo $event['current_wl']?></td><td><?php echo $event['quantity']?></td><td><?php echo $event['value']?></td><td><?php echo $event['unit']?></td><td><?php echo $event['warning']?></td><td><?php echo $event['alarm']?></td><td><?php echo $event['status']?></td><td><?php echo $event['rec_time']?></td><td><?php echo $event['remarks']?></td></tr>");
	<?php $st++;};?>
